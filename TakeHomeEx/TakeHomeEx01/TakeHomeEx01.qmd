...
for (i in seq_along(unique_dow)) {
  for (j in seq_along(unique_tod)) {
    ...
    map <- tm_shape(lixels_list[[cur_index]])+
        tm_lines(col="highway", lwd="density", scale=4.0, style = "quantile") +
...
```{r}
hist(filtered_rdacc_sf$time_of_day)
hist(filtered_rdacc_sf$dayofweek_name)
```
...
part 4 where the regions of high density also correlate with the high network kernel density here. Therefore, the time of day and day of week do not really matter for accidents, most likely since the thai metropolitian area is a well lit region of Thailand. 