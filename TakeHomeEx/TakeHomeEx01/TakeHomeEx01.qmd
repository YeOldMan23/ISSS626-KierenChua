...
## Part 6 : Check Network constrained

Now that we can see that there are clusters, we can put it in numbers by doing
lixelizing so that we can connect specific roads to the the locations.
...
filtered_rdacc_sf <- filtered_rdacc_sf %>% filter(weather_condition != "landslide") %>% filter(accident_type != "other")
```
...
loc_rdacc_sf <- filtered_rdacc_sf %>% 
  ...
  filter(dayofweek_name %in% wkdays)
...
colnames(bw_dataframe) <- c("Weekday", "Weekend")
...
for (i in seq_along(unique_accidents)) {
  for (j in c("Weekday", "Weekend")) {
    bw_dataframe[unique_accidents[i], j] <- count
    count <- count + 1
  }
}
...
```{r}
accidents_list <- vector("list", length(unique_accidents) * 2) # 2 cols
lixels_list <- vector("list", length(unique_accidents) * 2)
...
for (i in seq_along(unique_accidents)) {
  ...
  for (j in c("Weekday", "Weekend")) {
    if (j == "Weekday") {
      ...
      loc_rdacc_sf <- filtered_rdacc_sf %>% 
        filter(accident_type == unique_accidents[i]) %>%
        filter(dayofweek_name %in% wkends)
    }
...
for (i in seq_along(unique_accidents)) {
  for (j in c("Weekday", "Weekend")) {
    # Get the index
    cur_index <- bw_dataframe[unique_accidents[i], j]
    print(cur_index)
    set_title <- paste0(unique_accidents[i], " on ", j)
    
    map <- tm_shape(lixels_list[[cur_index]])+
        tm_lines(col="density", scale=4.0, style = "quantile", palette="Blues") +
...
# Print out the plots
tmap_mode('view')
tm_shape(lixels_list[[2]])+
  tm_lines(col="density", lwd="density", scale=4.0)+
tm_shape(accidents_list[[2]], name="Time of Day")+
  tm_dots(col = 'time_of_day', palette='BrBG') +
tm_shape(accidents_list[[2]], name="Vehicle Type") +
  tm_dots(col = 'vehicle_type', palette='Set2') + 
tm_shape(filter(accidents_list[[2]], dayofweek_name %in% c('Saturday', 'Sunday')), name = "Weekend")+
  tm_dots(col='red') + 
tm_shape(filter(accidents_list[[2]], dayofweek_name %in% c('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday')), name = "Weekday")+
  tm_dots(col='blue') + 
tm_shape(accidents_list[[2]], name="Quarter")+
  tm_dots(col = 'quarter', palette='viridis') +
tm_layout(title = unique_locs[2])
tmap_mode('plot')
```
...
# Print out the plots
tmap_mode('view')
tm_shape(lixels_list[[3]])+
  tm_lines(col="density", scale=4.0)+
tm_shape(accidents_list[[3]], name="Time of Day")+
  tm_dots(col = 'time_of_day', shape = "vehicle_type", palette='BrBG') +
tm_shape(filter(accidents_list[[3]], dayofweek_name %in% c('Saturday', 'Sunday')), name = "Weekend")+
  tm_dots(col='red') + 
tm_shape(filter(accidents_list[[3]], dayofweek_name %in% c('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday')), name = "Weekday")+
  tm_dots(col='blue') + 
tm_shape(accidents_list[[3]], name="Quarter")+
  tm_dots(col = 'quarter', palette='viridis') +
tm_layout(title = unique_locs[3])
tmap_mode('plot')
```
